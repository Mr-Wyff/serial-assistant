...
namespace serial_assistant
{
    ...
    public partial class MainWindow : Window
    {
        ...
        // 发送的方式
        public SendMode sendMode = SendMode.Character;
        ...
        */
            ...
            if (rb != null)
            {
                switch (rb.Tag.ToString())
                {
                    case "char":
                        ...
                        // 将文本框中内容转换成char
                        sendDataTextBox.Text = Utilities.ToSpecifiedText(sendDataTextBox.Text, SendMode.Character, Encoding.Default);
        ...
        private void loadFileRadioButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (File.Exists(textFile))
            {
                ...
                using (fs)
                {
                    TextRange text = new TextRange(recvDataRichTextBox.Document.ContentStart, recvDataRichTextBox.Document.ContentEnd);
                    text.Load(fs, DataFormats.Text);

                    sendDataTextBox.Text = text.Text;
                }
            }
        }
        ...
        */
        ...
        // 在尽可能短的时间内得到处理。
        private const int THRESH_VALUE = 1;
        ...
        private void ReceivedDataHandler(object obj)
        {
            ...
            this.Dispatcher.Invoke(new Action(() =>
            {
                ...
                {
                    // 根据显示模式显示接收到的字节.
                    recvDataRichTextBox.AppendText(Utilities.BytesToText(recvBuffer, receiveMode, Encoding.GetEncoding("GB2312")));
                    if ((bool)AutoChangeLine.IsChecked)
                        recvDataRichTextBox.AppendText("\n");
                    recvDataRichTextBox.ScrollToEnd();

                    RxCount.Text = rxcount.ToString();
                }
        ...
        private void RX_CountClear_Click(object sender, RoutedEventArgs e)
        {
            RxCount.Clear();
            rxcount = 0;
        }

        private void recvDataRichTextBoxClr_Click(object sender, RoutedEventArgs e)
        {
            recvDataRichTextBox.Document.Blocks.Clear();
            RxCount.Clear();
            rxcount = 0;
